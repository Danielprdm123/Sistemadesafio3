...
public class EntradaDAO {
	...
	public void update(Entrada en) throws SQLException {
		...
		Connection conexao = conn.getConnection();
		System.out.println(conn.getStatus());
		String sqlInsere = "UPDATE Entrada SET dataEntrada=(?),dataSaida=(?),statusEntrada=(?),situacaoPaciente=(?) where id =(?)";
		PreparedStatement stmt = conexao.prepareStatement(sqlInsere);

		stmt.setInt(5, en.getIdEntrada());
		stmt.setDate(1, java.sql.Date.valueOf(en.getDataEntrada()));
		stmt.setDate(2, java.sql.Date.valueOf(en.getDataDeSaida()));
